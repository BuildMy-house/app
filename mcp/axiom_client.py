...
class AxiomClient:
    ...
    def __init__(self, token: str | None = None) -> None:
        ...
        self.endpoint = (os.environ.get("AXIOM_URL") or os.environ.get("AXIOM_ENDPOINT") or "").rstrip("/")
        self.dataset = os.environ.get("AXIOM_DATASET", "buildmy-house-telemetry")

    def query(self, aql: str, *, timeframe: str = "24h") -> dict:
        if not self.endpoint or not self.token:
            raise RuntimeError("Axiom is not configured: AXIOM_URL/AXIOM_TOKEN must be set (no hardcoded fallback)")
